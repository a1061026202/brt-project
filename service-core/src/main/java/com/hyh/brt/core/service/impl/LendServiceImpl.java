...
import com.hyh.brt.core.mapper.LendMapper;
...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    @Override
    public List<Lend> selectList(Page<Lend> lendPage) {
        IPage<Lend> lends = baseMapper.selectPage(lendPage, null);

        List<Lend> records = lends.getRecords();
        records.forEach(lend -> {

            String returnMethod = dictService.getNameByParentDictCodeAndValues("returnMethod", lend.getReturnMethod());
    ...
    public void makeLoan(Long id) {
        ...
        //月年化
        BigDecimal montRate = lend.getServiceRate().divide(new BigDecimal(12), 8, BigDecimal.ROUND_DOWN);
        //平台服务费 = 已投金额*月年化* 投资时长
        BigDecimal realAmount = lend.getInvestAmount().multiply(montRate).multiply(new BigDecimal(lend.getPeriod()));
